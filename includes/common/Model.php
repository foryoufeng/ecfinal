...
abstract class CommonModel extends Model{
     ...
     * @return mixed
     */
    protected function page_and_size($where){
        ...
        if (I('page_size') && intval(I('page_size')) > 0)
        {
            $filter['page_size'] = intval(I('page_size'));
        }
        else
        {
            $filter['page_size'] = self::LIMIT;
        }

        /* 每页显示 */
        $filter['page'] = (empty(I('page')) || intval(I('page')) <= 0) ? 1 : intval(I('page'));
    ...
    public function common_nav_list(){
        ...
        if(session('role_ids')==-1){
            ...
            $action_ids=implode(',',M('roleaction')->where($info)->getField('id,action_id'));
            $action_where['action_id']=array('in',$action_ids);
            $parent_ids=array_unique(M('privilege')->where($action_where)->getField('action_id,parent_id'));
            $where['is_left_nav']=1;
            $where['action_id']=array('in',$action_ids.','.implode(',',$parent_ids));
            $data=M('privilege')->where($where)->order('sort desc,action_id asc')->select();

        }
        if($data){
            $data =  $this->getLayer($data);
        }
     ...
     */
    public function getLayer($types, $name = 'child', $pid = 0){
        $arr = array();
        foreach($types as $v){
            if($v['parent_id'] == $pid){
                $v[$name] = $this->getLayer($types, $name, $v['action_id']);//递归 把子类作为数组值压入数组中
                $arr[] = $v;
            }
        }
        return $arr;
    ...
    public function get_catid($cat_id){
        $data='';
        $temp=M('category')->where("parent_id in ($cat_id)")->select();
        if($temp){
            foreach($temp as $v){
                $data.=$v['cat_id'].',';
                $data.=$this->get_catid($v['cat_id']);
            }
        }
        return $data;
    }
    ...
    public function get_cat_id($cat_id){
        ...
        $data='';
        $where['parent_id'] = array('in',$cat_id);
        $temp = $this->table('__CATEGORY__')->field('cat_id,parent_id')->where($where)->select();
        if($temp){
            foreach($temp as $v){
                $data.=$v['cat_id'].',';
                $data.=$this->get_cat_id($v['cat_id']);
            }
        }
        return $data;
    }
...
abstract class BaseModel extends Model{
    ...
    public function page_and_size($count,$page_size=0){
        $filter['record_count']=$count;
        if ($page_size > 0)
        {
